...
func procedural_chunk_generation_method(chunk_id:Vector2i, path_type, path_out_dir:direction, generation_mode:int):
	...
	match generation_mode:
		0:
			...
			points.append(exit_point)
			add_points_to_path(points, path_type)
		1:  # Right-angled path generation with boundary constraints and no self-intersection
			var points = [extension_point]
			...
			points.append(exit_point)
			add_points_to_path(points, path_type)
		2:  # Multi-turn snake-like path with boundary constraints and no self-intersection
			var points = [extension_point]
			var current_point = extension_point
			...
			points.append(exit_point)
			add_points_to_path(points, path_type)
		3:  # Arc-based curved path generation with fixed center point
			var points = [extension_point]
			...
			points.append(exit_point)
			add_points_to_path(points, path_type)
		4:  # Pure diagonal zigzag path generation
			var points = [extension_point]
			var current_point = Vector3(extension_point.x, extension_point.y, extension_point.z)
			var exit_point_v3 = Vector3(exit_point.x, exit_point.y, exit_point.z)
			...
			add_points_to_path(points, path_type)
		_:
			# Default case if no specific generation mode is matched
			var points = [extension_point]
			points.append(exit_point)
			add_points_to_path(points, path_type)
